...
class BlockchainService {
  ...
  async initiateDispute(disputeData) {
    try {
      return await this.request('/disputes/initiate', {
        method: 'POST',
  ...
  async voteForArbitrator(voteData) {
    try {
      return await this.request('/disputes/vote', {
        method: 'POST',
        body: JSON.stringify(voteData),
      });
    } catch (error) {
      throw new Error(`Failed to vote for arbitrator: ${error.message}`);
    }
  }

  async makeArbitratorDecision(decisionData) {
    try {
      return await this.request('/disputes/decide', {
        method: 'POST',
        body: JSON.stringify(decisionData),
      });
    } catch (error) {
      throw new Error(`Failed to make arbitrator decision: ${error.message}`);
    }
  }
  ...
  async syncCrossChainData(syncData) {
    try {
      return await this.request('/cross-chain/sync', {
        method: 'POST',
        body: JSON.stringify(syncData),
      });
    } catch (error) {
      throw new Error(`Failed to sync cross-chain data: ${error.message}`);
    }
  }
